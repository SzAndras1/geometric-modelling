 ...
 */
class BesselPanel extends Panel
{
  /**
  ...
  */
  ...
  draw(context)
  {
    ...
    for (let i = 1; i < this._points.length; i++) {
      this.calculateLangrange(this._points[i-1], this._points[i])
      context.lineTo(this._points[i].x, this._points[i].y);
      console.log(i, this._points[i].x, this._points[i].y);
    }
    context.stroke();
  }

  calculateLangrange(point) {
